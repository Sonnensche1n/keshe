...
def rs_mat2s(poi, l_max):
    ...
    for i, loc1 in enumerate(candidate_loc):
        if i % 100 == 0:
            print(f"[兴趣点距离计算] 已处理位置点: {i}/{l_max}")
...
def process_traj(dname):  # start from 1
    ...
    for u_id in range(num_user+1):
        ...
        if u_id % 100 == 0:
            print(f"[用户轨迹处理] 用户ID: {u_id}，轨迹长度: {len(user_traj)}")
    ...
    # 整理输出结果
    data = [
        trajs,  # 用户轨迹张量 (NUM, M, 3)
        np.array(mat1),  # 用户空间+时间距离矩阵 (NUM, M, M, 2)
        mat2s,  # 所有兴趣点两两之间的空间距离 (L, L)
        np.array(mat2t),  # 用户轨迹的相对时间差 (NUM, M, M)
        labels,  # 预测目标 (NUM, M)
        np.array(lens),  # 每条轨迹的有效长度 (NUM)
        u_max,  # 用户ID最大值
        l_max  # 位置ID最大值
    ]

    # 保存到指定路径
    data_pkl_path = f'./data/{dname}_data.pkl'
    with open(data_pkl_path, 'wb') as pkl:
        joblib.dump(data, pkl)

    # 提示信息
    print("\n✅ 数据处理完成！以下是保存信息：")
    print(f"📁 路径：{os.path.abspath(data_pkl_path)}")
    print(f"📦 文件名：{os.path.basename(data_pkl_path)}")
    print("🚀 现在你可以运行 `train.py` 开始训练模型\n")
